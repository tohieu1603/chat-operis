// Operis Comms Page — Agent topology & inter-agent communication feed
'use strict';
...
function commsPage() {
  return {
    ...
    async loadData() {
      ...
      try {
        var results = await Promise.all([
          OperisAPI.get('/api/comms/topology'),
          OperisAPI.get('/api/comms/events?limit=200')
        ]);
    ...
    startSSE() {
      ...
      var self = this;
      var url = OperisAPI.baseUrl + '/api/comms/events/stream';
      if (OperisAPI.apiKey) url += '?token=' + encodeURIComponent(OperisAPI.apiKey);
    ...
    async refreshTopology() {
      try {
        this.topology = await OperisAPI.get('/api/comms/topology');
      } catch(e) { /* silent */ }
    },
    ...
    async submitSend() {
      ...
      try {
        await OperisAPI.post('/api/comms/send', {
          from_agent_id: this.sendFrom,
          to_agent_id: this.sendTo,
          message: this.sendMsg
        });
        OperisToast.success('Message sent');
        this.showSendModal = false;
      } catch(e) {
        OperisToast.error(e.message || 'Send failed');
      }
    ...
    async submitTask() {
      ...
      try {
        ...
        await OperisAPI.post('/api/comms/task', body);
        OperisToast.success('Task posted');
        this.showTaskModal = false;
      } catch(e) {
        OperisToast.error(e.message || 'Task failed');
      }
