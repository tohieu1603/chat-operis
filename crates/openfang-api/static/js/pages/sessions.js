// Operis Sessions Page — Session listing + Memory tab
'use strict';
...
function sessionsPage() {
  return {
    ...
    async loadSessions() {
      ...
      this.loadError = '';
      try {
        var data = await OperisAPI.get('/api/sessions');
    ...
    deleteSession(sessionId) {
      var self = this;
      OperisToast.confirm('Delete Session', 'This will permanently remove the session and its messages.', async function() {
        try {
          await OperisAPI.del('/api/sessions/' + sessionId);
          self.sessions = self.sessions.filter(function(s) { return s.session_id !== sessionId; });
          OperisToast.success('Session deleted');
        } catch(e) {
          OperisToast.error('Failed to delete session: ' + e.message);
        }
      });
    },
    ...
    async loadKv() {
      ...
      this.memLoadError = '';
      try {
        var data = await OperisAPI.get('/api/memory/agents/' + this.memAgentId + '/kv');
    ...
    async addKey() {
      ...
      try {
        await OperisAPI.put('/api/memory/agents/' + this.memAgentId + '/kv/' + encodeURIComponent(this.newKey), { value: value });
        this.showAdd = false;
        OperisToast.success('Key "' + this.newKey + '" saved');
        this.newKey = '';
        this.newValue = '""';
        await this.loadKv();
      } catch(e) {
    ...
    deleteKey(key) {
      var self = this;
      OperisToast.confirm('Delete Key', 'Delete key "' + key + '"? This cannot be undone.', async function() {
        try {
          await OperisAPI.del('/api/memory/agents/' + self.memAgentId + '/kv/' + encodeURIComponent(key));
          OperisToast.success('Key "' + key + '" deleted');
          await self.loadKv();
        } catch(e) {
          OperisToast.error('Failed to delete key: ' + e.message);
        }
      });
    },
    ...
    async saveEdit() {
      ...
      try {
        await OperisAPI.put('/api/memory/agents/' + this.memAgentId + '/kv/' + encodeURIComponent(this.editingKey), { value: value });
        OperisToast.success('Key "' + this.editingKey + '" updated');
        this.editingKey = null;
        this.editingValue = '';
        await this.loadKv();
      } catch(e) {
