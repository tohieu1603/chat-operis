...
impl OpenFangKernel {
    ...
    pub fn boot_with_config(mut config: KernelConfig) -> KernelResult<Self> {
        ...
        let driver_config = DriverConfig {
            ...
            api_key: config.default_model.api_key.clone()
    ...
    fn resolve_driver(&self, manifest: &AgentManifest) -> KernelResult<Arc<dyn LlmDriver>> {
        ...
        let primary = if agent_provider == default_provider && !has_custom_key && !has_custom_url {
            ...
            let api_key = if has_custom_key {
                // Agent explicitly set an API key env var — try it, fallback to config api_key
                manifest
                    .model
                    .api_key_env
                    .as_ref()
                    .and_then(|env| std::env::var(env).ok())
                    .or_else(|| self.config.default_model.api_key.clone())
            } else if agent_provider == default_provider {
                // Same provider — use default key (direct api_key takes priority)
                self.config.default_model.api_key.clone()
                    .or_else(|| std::env::var(&self.config.default_model.api_key_env).ok())
            } else {
